  ...
  const textos = {
    es: {
      ...
      buzon: {
        ...
        descripcion: "Estamos preparando el Buzón de SénecAI para que puedas compartir tus preguntas y recibir respuestas personalizadas con la sabiduría de Séneca."
      },
      footer: { derechos: "© 2024 SenecAI. Todos los derechos reservados." }
    },
    en: {
      home: {
        titulo: "SenecAI",
      ...
      buzon: {
        ...
        descripcion: "We are preparing the SenecAI Mailbox so you can share your questions and receive personalized responses with Seneca's wisdom."
      },
      footer: { derechos: "© 2024 SenecAI. All rights reserved." }
    }
  };
            ...
            <div className="max-w-4xl mx-auto px-6">
              ...
              {piezaSeleccionada && piezaSeleccionada.tipo === 'carta' ? (
                <article>
                  ...
                  <div className="bg-yellow-100 border-2 border-yellow-800 rounded-lg p-8 shadow-md max-w-3xl mb-12">
                    <h3 className="text-3xl font-bold text-yellow-900 mb-6 font-serif">{piezaSeleccionada.pieza.titulo[idioma]}</h3>
                  ...
                  <div className="bg-yellow-50 p-8 rounded border border-yellow-200 max-w-3xl">
                    <iframe src="https://diestrodeoido.substack.com/embed" width="100%" height="320" style={{border: '1px solid #EEE', background: 'white', borderRadius: '8px'}} frameBorder="0" scrolling="no"></iframe>
                  </div>
                </article>
              ) : (
            ...
            <div className="max-w-4xl mx-auto px-6">
              ...
              {piezaSeleccionada && piezaSeleccionada.tipo === 'email' ? (
                <article>
                  ...
                  <div className="bg-white border border-gray-300 rounded-lg shadow-sm max-w-3xl mb-12">
                    <div className="border-b border-gray-200 p-6 bg-gray-50">
                      <div className="text-sm text-gray-600 font-mono"><p><span className="font-semibold">{idioma === 'es' ? 'Asunto' : 'Subject'}:</span> {piezaSeleccionada.pieza.titulo[idioma]}</p><p><span className="font-semibold">{idioma === 'es' ? 'Fecha' : 'Date'}:</span> {piezaSeleccionada.pieza.fecha}</p></div>
                    </div>
                ...
                <div className="grid gap-6">
                  {piezas.emails.slice().reverse().map((email) => (
                    <div key={email.id} className="bg-white p-6 rounded border border-gray-200 hover:border-yellow-400 transition cursor-pointer" onClick={() => setPiezaSeleccionada({tipo: 'email', pieza: email})}>
                      <h3 className="text-xl font-bold text-gray-900 mb-2">{email.titulo[idioma]}</h3>
                      <p className="text-gray-600 text-sm leading-relaxed mb-4">{email.contenido[idioma].split('\n\n')[0].substring(0, 150)}...</p>
                      <button className="mt-4 text-yellow-600 hover:text-yellow-700 font-medium text-sm">{idioma === 'es' ? 'Leer más →' : 'Read more →'}</button>
                    </div>
                  ))}
                </div>
            ...
            <div className="max-w-2xl mx-auto px-6">
              ...
              <div className="bg-white p-8 rounded border border-gray-200">
                <div className="text-center py-12">
                  ...
                  <h3 className="text-2xl font-bold text-gray-900 mb-3">{t.buzon.titulo}</h3>
                  <p className="text-gray-600 mb-8">{t.buzon.descripcion}</p>
                </div>
              </div>
              <div className="mt-12 bg-yellow-50 p-8 rounded border border-yellow-200">
                <iframe src="https://diestrodeoido.substack.com/embed" width="100%" height="320" style={{border: '1px solid #EEE', background: 'white', borderRadius: '8px'}} frameBorder="0" scrolling="no"></iframe>
              </div>
            </div>
          </section>
